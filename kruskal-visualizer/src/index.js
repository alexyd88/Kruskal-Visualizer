...
function Graph(props) {
    ...
    function refreshData()
    {
        ...
        let t5 = header.insertCell(4);
        let t6 = header.insertCell(5);
        t1.innerText = 'COST';
        t2.innerText = 'A';
        t3.innerText = 'B';
        t4.innerText = 'NOTE';
        ...
        for (let i = 0; i < edges.length; i++)
        {
            ...
            c1.innerText = edges[i].weight;
            c2.innerText = edges[i].to;
            c3.innerText = edges[i].from;
            if (!vish[step][i] && pah[step][edges[i].to] != pah[step][edges[i].from] && !foundFirst)
            {
                foundFirst = true;
                c4.innerText = "MINIMUM EDGE WEIGHT FOR UNCONNECTED NODES, WILL ADD NEXT"
            }
            else if (vish[step][i])
            {
                c4.innerText = "ALREADY ADDED"
            }
            else if (pah[step][edges[i].to] == pah[step][edges[i].from])
            {
                c4.innerText = "ALREADY CONNECTED"
            }
        }
        let ff = false;
    }
